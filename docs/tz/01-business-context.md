...
- **Скоро:** API с ключом GUID из 1С: несколько фото на товар, РРЦ с НДС, числовые остатки по складам, характеристики «ключ: значение» одной строкой, бренд, вид номенклатуры и страна. Дилерские цены и признак снятия с производства поставщик прорабатывает. Документация (OpenAPI и регламент обновления) готовится, сроков нет (ответ поставщика от 17.09.2026, `docs/supplier-api-2026-09-17.md`).
...
**Цель релиза:** работающая витрина с каталогом поставщика, актуальным наличием, поиском, фильтрами, корзиной, оформлением заявки, B2B-кабинетом и админкой, развёрнутая на VPS в РФ (§17.1).
