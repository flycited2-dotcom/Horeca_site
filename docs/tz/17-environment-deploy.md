...
| Счета PDF | пока `local` — диск сервера (`INVOICES_DISK`) | отдаются только после проверки прав (§15.7) |

**Проверено 22.09.2026:** бакет `s3-968732` Спринтхост отдаёт всем целиком — файл с правами «только владелец» всё равно открывается по прямой ссылке. Поэтому счета, где есть данные клиентов, лежат на диске сервера (§15.10), а в хранилище — только фото, которые и так публичны. Появится отдельный приватный бакет — хватит вписать `AWS_PRIVATE_BUCKET` и `INVOICES_DISK=s3-private`, диск `s3-private` для этого готов.
