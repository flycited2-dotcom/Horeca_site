...
13. **Фото и характеристик** в XML нет, описание обрезано на 500 символах. Регулярные выражения по названиям дают мощность у 2% товаров и габариты у 14%, поэтому извлечение не делается: фото, полные описания, габариты и характеристики берутся со списка товаров сайта (п. 9), позже — из API.
...
9. **Фото, описание, габариты и характеристики.** Пока нет API — со списка товаров сайта rosholod.org (`/api/v1/prices/`; фото — решение заказчика-дилера от 23.09.2026, остальное — от 24.09.2026): адаптер `RosholodSiteSource` за интерфейсом `SupplierContentSourceInterface`, команда `supplier:content {--page} {--force}` (по расписанию — в 00:30) ставит в очередь `imports` задачи по странице из 20 товаров; пока идёт прежняя загрузка, новая не начинается, `--force` начинает заново и останавливает прежнюю (метка загрузки — в кэше); страница — раз в секунду, пауза 250 мс перед каждым фото, запросы подписаны названием магазина, берутся только фото из папки поставщика.
   - **Сведения** (`SyncSupplierDetails`): полное описание (заголовок «Описание», приклеенный к тексту, убирается), длина, ширина (или глубина, если ширины нет), высота, вес, гарантия — в поля товара; остальное — в характеристики через `AttributeValueParser` (прочерк «-» — нет значения): единица — после запятой в названии, число — только при единице и числовом значении, иначе текст; страна производства — характеристикой. Бренд со списка не берётся — он приходит с каталогом. Новая характеристика создаётся нефильтруемой и не главной. Пустое у поставщика не стирает заполненное; характеристика поставщика, пропавшая у товара, снимается; поля из `locked_fields` и характеристики с `source = manual` не трогаются. Описанием, габаритами, весом и гарантией владеет этот источник: каталог XML заполняет описание только у нового товара (`seededProductFields`). `SyncSupplierPhotos` скачивает набор фото целиком и только потом заменяет прежние фото поставщика; тот же набор повторно не скачивается (`source_url`). Фото помечаются `source = supplier` — API заменит их тем же путём, сменив адаптер. Ручные фото (`source = manual`) не удаляются и остаются первыми. Уменьшенные копии делаются в той же задаче, очередь писем не занимают.
