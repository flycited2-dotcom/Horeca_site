...
7. **Тип цены — РРЦ**, подтверждено заказчиком 16.09.2026 (`suppliers.price_kind = rrp`). API первой версии, по словам поставщика, тоже отдаёт РРЦ; дилерские цены появятся позже.
