...
9. **Фото.** Пока нет API — со списка товаров сайта rosholod.org (`/api/v1/prices/`, решение заказчика-дилера от 23.09.2026): адаптер `RosholodSitePhotoSource` за интерфейсом `SupplierPhotoSourceInterface`, команда `supplier:photos {--page}` ставит в очередь `imports` задачи по странице из 20 товаров, страница — раз в секунду, пауза 250 мс перед каждым фото, запросы подписаны названием магазина, берутся только фото из папки поставщика. `SyncSupplierPhotos` скачивает набор фото целиком и только потом заменяет прежние фото поставщика; тот же набор повторно не скачивается (`source_url`). Фото помечаются `source = supplier` — API заменит их тем же путём, сменив адаптер. Ручные фото (`source = manual`) не удаляются и остаются первыми. Уменьшенные копии делаются в той же задаче, очередь писем не занимают.
