...
9. **Фото, описание, габариты и характеристики.** Пока нет API — со списка товаров сайта rosholod.org (`/api/v1/prices/`; фото — решение заказчика-дилера от 23.09.2026, остальное — от 24.09.2026): адаптер `RosholodSiteSource` за интерфейсом `SupplierContentSourceInterface`, команда `supplier:content {--page}` (по расписанию — в 00:30) ставит в очередь `imports` задачи по странице из 20 товаров, страница — раз в секунду, пауза 250 мс перед каждым фото, запросы подписаны названием магазина, берутся только фото из папки поставщика.
