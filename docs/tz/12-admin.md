...
- **Товары.**
  ...
  - Форма — вкладки: Основное · Цены (включая `product_prices`) · Параметры · SEO; склады (только чтение) и характеристики — блоками под формой; фото — отдельной вкладкой. Замок у полей из `locked_fields` и действие «Вернуть значение поставщика».
