...
- **Серверный рендеринг (Blade + Livewire).** Каталог живёт на органическом поиске («пароконвектомат купить Симферополь»). Страницы отдаются готовым HTML без Node на проде. На сервере магазин — PHP-FPM в своих контейнерах Docker (§17.1).
...
- **MariaDB 11.8 LTS** — в контейнере магазина рядом с сайтом (§17.1). Код обязан работать и на MySQL 8.4 LTS, поэтому: без FULLTEXT-парсера `ngram` и без JSON-функций, специфичных для одной СУБД, в запросах. SQLite запрещён во всех окружениях, включая тесты.
...
league/flysystem-aws-s3-v3 ^3.35   фото и счета в S3-хранилище (§17.9); из AWS SDK оставлен только S3
