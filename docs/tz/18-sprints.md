...
- *Сделано заранее, 18.09.2026, ветка `feature/sprint-3-catalog`* — бэкенд, не зависящий от макетов: ресурсы Filament «Товары», «Категории», «Бренды» по §12 (замки ручных правок, «Вернуть значение поставщика», фильтры, массовые действия, выгрузка в XLSX, объединение брендов), 301-редиректы при смене адресов, дашборд с последними прогонами импорта и тем, что каталог ждёт от менеджера. 311 тестов.
